...
void icm20948GyroRawRead(int16_t* ps16X, int16_t* ps16Y, int16_t* ps16Z);
void icm20948AccelRawRead(int16_t* ps16X, int16_t* ps16Y, int16_t* ps16Z);
...
// ---- Unfiltered burst reads ----
// Identical transaction to the Fast variants, minus the 8-tap moving average.
// A recording has to keep the bandwidth it paid for; smoothing is a decision the
// analysis makes afterwards, on data it can still choose not to smooth.
void icm20948AccelRawRead(int16_t* ps16X, int16_t* ps16Y, int16_t* ps16Z)
{
    uint8_t b[6];
    icm20948ReadBurst(REG_ADD_ACCEL_XOUT_H, b, 6);

    *ps16X = (int16_t)((b[0] << 8) | b[1]);
    *ps16Y = (int16_t)((b[2] << 8) | b[3]);
    *ps16Z = (int16_t)((b[4] << 8) | b[5]);
    return;
}
...
void icm20948GyroRawRead(int16_t* ps16X, int16_t* ps16Y, int16_t* ps16Z)
{
    ...
    // Zero-rate offset measured once at boot. It is a constant, not a filter, so
    // it does not cost bandwidth and it stays.
    *ps16X = (int16_t)((int32_t)rawX - gstGyroOffset.s16X);
    *ps16Y = (int16_t)((int32_t)rawY - gstGyroOffset.s16Y);
    *ps16Z = (int16_t)((int32_t)rawZ - gstGyroOffset.s16Z);
    return;
}
